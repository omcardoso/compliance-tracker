...
// ─── Email Logs ───────────────────────────────────────────────────────────────
export async function fbLogEmail(log) {
  const id = "log_" + Date.now() + "_" + Math.random().toString(36).slice(2,6);
  await setDoc(doc(db, "emailLogs", id), { ...log, id, sentAt: new Date().toISOString() });
}
export async function fbGetEmailLogs(companyName) {
  const q = query(collection(db, "emailLogs"), where("companyName", "==", companyName));
  const snap = await getDocs(q);
  return snap.docs.map(d => d.data()).sort((a,b) => new Date(b.sentAt) - new Date(a.sentAt));
}
