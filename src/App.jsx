...
const STATUS_STYLES = {
  "Not Started":    { bg:"#1e293b", color:"#cbd5e1" },
  "In Progress":    { bg:"#1e3a5f", color:"#93c5fd" },
  "Waiting Client": { bg:"#422006", color:"#fcd34d" },
  "Complete":       { bg:"#052e16", color:"#6ee7b7" },
  "Pending":        { bg:"#1e293b", color:"#cbd5e1" },
  "Done":           { bg:"#052e16", color:"#6ee7b7" },
  "Overdue":        { bg:"#450a0a", color:"#fca5a5" },
  "No Filings":     { bg:"#1e293b", color:"#475569" },
};

// ─── Firebase helpers ─────────────────────────────────────────────────────────
const FILINGS_COL = "filings";

async function fbGetFilings(year) {
  const q = year
    ? query(collection(db, FILINGS_COL), where("year", "==", parseInt(year)))
    : collection(db, FILINGS_COL);
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ ...d.data(), filingId: d.id }));
}

async function fbSaveFiling(filing) {
  await setDoc(doc(db, FILINGS_COL, filing.filingId), filing);
}

async function fbDeleteFiling(filingId) {
  await deleteDoc(doc(db, FILINGS_COL, filingId));
}

async function fbDeleteYear(year) {
  const q = query(collection(db, FILINGS_COL), where("year", "==", parseInt(year)));
  const snap = await getDocs(q);
...
function Btn({ onClick, disabled, children, variant="primary", size="md", style={} }) {
  const base = { border:"none", borderRadius:7, cursor:disabled?"default":"pointer", fontWeight:800,
...
function EmailModal({ filing, company, onClose }) {
  ...
  const inp = { width:"100%", background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7,
    color:C.text, padding:"7px 10px", fontSize:13, fontFamily:"inherit", outline:"none", boxSizing:"border-box" };
  const lbl = { fontSize:10, fontWeight:800, color:C.text3, textTransform:"uppercase", letterSpacing:"0.06em", display:"block", marginBottom:3 };
  const send = async () => {
      ...
      {sent ? <div style={{ textAlign:"center", padding:40 }}><div style={{ fontSize:48, marginBottom:12 }}>✓</div><p style={{ fontSize:15, fontWeight:700, color:C.success }}>Email sent!</p></div> : (
        <div>
          {[["To",to,setTo],["Subject",subject,setSubject]].map(([label,val,setter])=>(
            <div key={label} style={{ marginBottom:12 }}><label style={lbl}>{label}</label><input value={val} onChange={e=>setter(e.target.value)} style={inp}/></div>
          ))}
          <div style={{ marginBottom:16 }}><label style={lbl}>Body</label>
            <textarea value={body} onChange={e=>setBody(e.target.value)} rows={12} style={{...inp,resize:"vertical",lineHeight:1.6}}/>
          </div>
          <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
            <Btn variant="secondary" onClick={onClose}>Cancel</Btn>
            <Btn onClick={send} disabled={sending||!to.trim()}>{sending?<><Spinner size={13} color="#fff"/>Sending...</>:"Send Email"}</Btn>
          </div>
        </div>
      )}
    </Modal>
...
function StepRow({ step, users, isAdmin, currentUserEmail, onUpdate }) {
  ...
  const selStyle = { background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7, color:C.text, padding:"5px 8px", fontSize:12, outline:"none", width:"100%" };

  const quickToggle = () => {
    if (!canEdit) return;
    onUpdate({ ...step, status:isDone?"Pending":"Done", completedAt:isDone?"":new Date().toISOString() });
  };
  const save = async () => {
    setSaving(true);
    onUpdate({ ...step, status, notes, assignedTo, completedAt:status==="Done"?(step.completedAt||new Date().toISOString()):"" });
    setEditing(false);
    setSaving(false);
  };
  const assignLabel = step.assignedTo==="__client__" ? "Client"
    : (users.find(u=>u.email===step.assignedTo)||{}).name?.split(" ")[0] || step.assignedTo?.split("@")[0];

  return (
    <div style={{ padding:"9px 14px", borderBottom:"1px solid "+C.border, background:isDone?"#0d1f13":"transparent" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        <div onClick={quickToggle} style={{ width:18, height:18, borderRadius:4, flexShrink:0, cursor:canEdit?"pointer":"default",
          background:isDone?C.success:"transparent", border:"2px solid "+(isDone?C.success:C.border2),
      ...
      {editing && (
        <div style={{ marginTop:10, padding:12, background:C.card2, borderRadius:8, border:"1px solid "+C.border }}>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
            {isAdmin && (
              <div style={{ flex:"1 1 160px" }}>
                <label style={{ fontSize:10, fontWeight:800, color:C.text3, textTransform:"uppercase", display:"block", marginBottom:3 }}>Assign to</label>
            ...
            <div style={{ flex:"1 1 130px" }}>
              <label style={{ fontSize:10, fontWeight:800, color:C.text3, textTransform:"uppercase", display:"block", marginBottom:3 }}>Status</label>
          ...
          <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={2} placeholder="Notes..."
            style={{ width:"100%", background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7, color:C.text,
              padding:"6px 8px", fontSize:12, fontFamily:"inherit", outline:"none", resize:"vertical", boxSizing:"border-box", marginBottom:8 }}/>
          <div style={{ display:"flex", gap:6 }}>
            <Btn variant="secondary" size="sm" onClick={()=>setEditing(false)}>Cancel</Btn>
            <Btn size="sm" onClick={save} disabled={saving}>{saving?<><Spinner size={10} color="#fff"/>Saving...</>:"Save"}</Btn>
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{ border:"1px solid "+C.border, borderRadius:10, overflow:"hidden", marginBottom:10 }}>
      ...
      {expanded && steps.map(step=>(
        <StepRow key={step.stepId} step={step} users={users} isAdmin={isAdmin}
          currentUserEmail={currentUserEmail} onUpdate={updateStep}/>
      ))}
    </div>
  );
}
...
function YearManager({ companies, filings, setFilings, yearFilter, setYearFilter, onClose, showToast }) {
  ...
  const selStyle = { background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7, color:C.text, padding:"7px 10px", fontSize:13, outline:"none", width:"100%", marginBottom:10 };
    ...
    for (const c of companies) {
      ...
      for (const ft of toCreate) {
        const steps = getSteps(ft, c.jurisdiction).map((name,i)=>({
          stepId:uid(), stepName:name, assignedTo:"", status:"Pending", notes:"", completedAt:"", order:i
        }));
        const f = { filingId:uid(), companyName:c.name, jurisdiction:c.jurisdiction, filingType:ft,
          year:parseInt(newYear), status:"Not Started", dueDate:getDueDate(ft,parseInt(newYear)), steps, yearNotes:"" };
        await fbSaveFiling(f);
        created++;
  ...
  return (
    <Modal title="Manage Filing Years" onClose={onClose} width={520}>
      <div style={{ background:C.card2, borderRadius:10, padding:16, marginBottom:16, border:"1px solid "+C.border }}>
        ...
        <select value={newYear} onChange={e=>setNewYear(e.target.value)} style={selStyle}>
        ...
        <p style={{ fontSize:11, color:C.text3, marginBottom:12, lineHeight:1.5 }}>
          Creates all required filings for every company based on their jurisdiction.
        </p>
        {progress && <p style={{ fontSize:11, color:C.accent, marginBottom:8 }}>{progress}</p>}
        <Btn onClick={initAll} disabled={working}>
      ...
      <div style={{ background:"#1a0a0a", borderRadius:10, padding:16, border:"1px solid #7f1d1d" }}>
        ...
        <select value={delYear} onChange={e=>setDelYear(e.target.value)} style={{...selStyle,background:"#1a0a0a",borderColor:"#7f1d1d"}}>
...
function EmailBlastModal({ companies, filings, yearFilter, onClose }) {
  ...
  const sendAll = async () => {
    ...
    for (const g of groups) {
      const to = editTo[g.filing.filingId]!==undefined ? editTo[g.filing.filingId] : g.to;
      if (!to) continue;
      const { subject, body } = getTmpl(g);
      try { await apiWrite({ action:"sendComplianceEmail", to, subject, body }); count++; setSent(count); } catch(e) {}
    }
    setDone(true); setSending(false);
  };

  const inp = { width:"100%", background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7,
    color:C.text, padding:"5px 8px", fontSize:12, fontFamily:"inherit", outline:"none", boxSizing:"border-box" };
    ...
    <Modal title={"Email Clients \u2014 Tax Year "+taxYear} onClose={onClose} width={720}>
      {done ? (
        ...
        <div style={{ textAlign:"center", padding:40 }}>
          ...
          <p style={{ fontSize:12, color:C.text3, marginTop:8 }}>Assign steps to "Client" to include them in the email blast.</p>
        </div>
      ) : (
        <div>
          <div style={{ background:C.card2, borderRadius:9, padding:"10px 14px", marginBottom:16, fontSize:12, color:C.text3, border:"1px solid "+C.border }}>
            <span style={{ color:C.text, fontWeight:700 }}>{groups.length} emails</span> will be sent to clients with pending action items for Tax Year {taxYear}.
          </div>
          {groups.map(g=>{
            const to = editTo[g.filing.filingId]!==undefined?editTo[g.filing.filingId]:(g.to||"");
            return (
              <div key={g.filing.filingId} style={{ background:C.card2, borderRadius:9, padding:12, marginBottom:10, border:"1px solid "+C.border }}>
                ...
                <div style={{ marginBottom:6 }}>
                  <label style={{ fontSize:10, fontWeight:800, color:C.text3, textTransform:"uppercase", display:"block", marginBottom:3 }}>To</label>
                  <input value={to} onChange={e=>setEditTo(p=>({...p,[g.filing.filingId]:e.target.value}))} placeholder="Client email" style={{...inp,borderColor:to?C.border2:C.danger}}/>
                  {!to&&<div style={{ fontSize:10, color:C.danger, marginTop:2 }}>No email — will be skipped</div>}
                </div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                  {g.steps.map(s=><span key={s.stepId} style={{ fontSize:10, background:"#1e293b", color:"#94a3b8", borderRadius:5, padding:"1px 7px" }}>{s.stepName}</span>)}
                </div>
              </div>
            );
          })}
          <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:16 }}>
            ...
            <Btn onClick={sendAll} disabled={sending}>
              {sending?<><Spinner size={13} color="#fff"/>Sending {sent}/{groups.length}...</>:"Send "+groups.length+" Email"+(groups.length>1?"s":"")}
            </Btn>
          </div>
        </div>
      )}
...
function SidePanel({ company, filings, users, isAdmin, currentUserEmail, yearFilter, onClose, updateFiling }) {
  ...
  const saveNotes = async () => {
    ...
    const nr = existing
      ...
      : { filingId:"notes_"+company.name.replace(/[^a-z0-9]/gi,"_")+"_"+yearFilter,
          companyName:company.name, jurisdiction:company.jurisdiction, filingType:"__notes__",
          year:parseInt(yearFilter), status:"N/A", dueDate:"", steps:[], yearNotes:notes };
    await fbSaveFiling(nr);
    updateFiling(company.name, nr);
    setNotesSaved(true);
  ...
  return (
    <div style={{ width:"58%", maxWidth:820, background:C.card, borderLeft:"1px solid "+C.border,
      display:"flex", flexDirection:"column", overflowY:"auto", flexShrink:0 }}>
      <div style={{ padding:"14px 16px", background:C.card2, borderBottom:"1px solid "+C.border,
        display:"flex", alignItems:"flex-start", gap:12, position:"sticky", top:0, zIndex:10 }}>
        <div style={{ flex:1 }}>
      ...
      <div style={{ padding:16, flex:1 }}>
        ...
        <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3}
          placeholder={"Notes for "+company.name+" (Tax Year "+taxYear+")..."}
          style={{ width:"100%", background:C.inputBg, border:"1px solid "+C.border2, borderRadius:8,
            color:C.text, padding:"8px 10px", fontSize:12, fontFamily:"inherit", outline:"none",
            resize:"vertical", boxSizing:"border-box", lineHeight:1.6, marginBottom:8 }}/>
        ...
        {realFilings.length===0 ? (
          <div style={{ textAlign:"center", padding:32, color:C.text3, fontSize:13 }}>No filings for {yearFilter} yet.</div>
...
export default function App() {
  ...
  const loadData = useCallback(async()=>{
    ...
    try {
      ...
      // Load companies from Apps Script (source of truth)
      // Load compliance filings from Firestore (fast)
      const [cd, ud, filingDocs] = await Promise.all([
        apiRead("getCompanies"),
        apiRead("getUsers"),
        fbGetFilings(yearFilter),
      ]);
      const ul = ud.users||[];
      ...
      const fm={};
      filingDocs.forEach(f=>{ if(!fm[f.companyName])fm[f.companyName]=[]; fm[f.companyName].push(f); });
      setFilings(fm);
  ...
  const updateFiling = (companyName, updatedFiling) => {
    setFilings(prev=>{
      const list = prev[companyName]||[];
      const exists = list.some(f=>f.filingId===updatedFiling.filingId);
      return { ...prev, [companyName]: exists ? list.map(f=>f.filingId===updatedFiling.filingId?updatedFiling:f) : [...list,updatedFiling] };
    });
  };
  ...
  const hdrSpan = (label, col) => (
    <span onClick={col?()=>sortCol(col):undefined}
      style={{ fontSize:9, fontWeight:900, color:C.text3, textTransform:"uppercase",
        letterSpacing:"0.07em", cursor:col?"pointer":"default" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", color:C.text }}>
      {/* Header */}
      <div style={{ background:"#000", borderBottom:"1px solid "+C.border, padding:"0 20px",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <select value={yearFilter} onChange={e=>{setYearFilter(e.target.value);setSelectedCo(null);}}
            style={{ background:"#111", color:C.text, border:"1px solid "+C.border2, borderRadius:7,
              padding:"4px 10px", fontSize:12, fontWeight:700, outline:"none", colorScheme:"dark" }}>
            {[CURRENT_YEAR+1,CURRENT_YEAR,CURRENT_YEAR-1,CURRENT_YEAR-2].map(y=><option key={y} value={y}>{y}</option>)}
          </select>
          <span style={{ fontSize:11, color:C.text3, background:C.card2, border:"1px solid "+C.border, borderRadius:6, padding:"3px 9px", whiteSpace:"nowrap" }}>
            Tax Year <span style={{ color:"#818cf8", fontWeight:800 }}>{taxYear}</span>
          </span>
        </div>
        {isAdmin && <>
          <Btn size="sm" variant="secondary" onClick={()=>setYearMgr(true)}>🗓 Manage Years</Btn>
          <Btn size="sm" variant="secondary" onClick={()=>setEmailBlast(true)} style={{ color:"#34d399", borderColor:"#064e3b" }}>📧 Email Clients</Btn>
        </>}
        <UserButton/>
      </div>
      ...
      <div style={{ background:"#111", borderBottom:"1px solid "+C.border, padding:"8px 20px", display:"flex", gap:6, alignItems:"center" }}>
        {[{v:stats.total,l:"Total",c:"#818cf8"},{v:stats.complete,l:"Complete",c:"#34d399"},{v:stats.overdue,l:"Overdue",c:"#f87171"},{v:stats.dueSoon,l:"Due Soon",c:"#fbbf24"},{v:myTasks.length,l:"My Tasks",c:"#a78bfa"}].map(s=>(
          <React.Fragment key={s.l}>
            ...
            <span style={{ fontSize:10,color:C.text3,marginRight:14 }}>{s.l}</span>
          </React.Fragment>
        ))}
      </div>
      ...
      <div style={{ display:"flex", height:"calc(100vh - 104px)" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", minWidth:0 }}>
            ...
            <div style={{ position:"relative", flex:"1 1 180px", maxWidth:240 }}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search companies..."
                style={{ width:"100%", background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7,
                  color:C.text, padding:"6px 8px 6px 26px", fontSize:12, outline:"none",
                  boxSizing:"border-box", fontFamily:"inherit", colorScheme:"dark" }}/>
            </div>
            {[
              [statusFilter,setStatusFilter,["All",...FILING_STATUSES,"No Filings","Overdue"],"Status"],
              [jurFilter,setJurFilter,jurOptions,"Jurisdiction"],
              [typeFilter,setTypeFilter,["All",...ALL_FILING_TYPES],"Filing Type"],
              ...
              <select key={i} value={val} onChange={e=>setter(e.target.value)}
                style={{ background:C.inputBg, border:"1px solid "+C.border2, borderRadius:7,
                  color:C.text, padding:"6px 9px", fontSize:12, outline:"none", colorScheme:"dark" }}>
          ...
          {/* Column headers */}
          <div style={{ display:"grid", gridTemplateColumns:panelOpen?gridComp:gridFull,
            padding:"6px 14px", background:"#111", borderBottom:"1px solid "+C.border,
            position:"sticky", top:53, zIndex:9 }}>
            {panelOpen ? <>
              {hdrSpan("Company","name")}
              {hdrSpan("Jurisdiction","jurisdiction")}
              {hdrSpan("Status","status")}
            </> : <>
              {hdrSpan("Company","name")}
              {hdrSpan("Jurisdiction","jurisdiction")}
              {hdrSpan("Filing Types",null)}
              {hdrSpan("Status","status")}
              {hdrSpan("Next Responsible",null)}
              {hdrSpan("",null)}
            </>}
          </div>
          ...
          {visibleCompanies.length===0 ? (
            <div style={{ padding:48, textAlign:"center", color:C.text3, fontSize:13 }}>No companies match your filters.</div>
            ...
            return (
              <div key={c.name} onClick={()=>setSelectedCo(isSel?null:c)}
                style={{ display:"grid", gridTemplateColumns:panelOpen?gridComp:gridFull,
                  padding:"10px 14px", borderBottom:"1px solid "+C.border,
                  cursor:"pointer", background:isSel?"#1e1b4b":"transparent", transition:"background 0.1s" }}
                onMouseEnter={e=>{if(!isSel)e.currentTarget.style.background=C.card2;}}
                onMouseLeave={e=>{if(!isSel)e.currentTarget.style.background="transparent";}}>
                <div><div style={{ fontWeight:700,fontSize:12,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{c.name}</div>
                  {c.clientEmail&&<div style={{ fontSize:10,color:C.text3 }}>{c.clientEmail}</div>}</div>
                ...
                {!panelOpen && (
                  <div style={{ display:"flex",flexDirection:"column",gap:2,justifyContent:"center" }}>
                    {getFilingTypes(c.jurisdiction).map(ft=>(
                      <span key={ft} style={{ fontSize:9,background:C.card2,color:C.text3,borderRadius:4,padding:"1px 5px",display:"inline-block" }}>{ft}</span>
                    ))}
                  </div>
                )}
                <div style={{ display:"flex",alignItems:"center" }}>
                  {hasFilings?<Badge status={ws}/>:<span style={{ fontSize:10,color:C.text3,fontStyle:"italic" }}>No filings</span>}
                </div>
                {!panelOpen && (
                  <div style={{ display:"flex",flexDirection:"column",justifyContent:"center" }}>
                    {next ? <>
                      <span style={{ fontSize:11,fontWeight:700,color:"#818cf8" }}>{next.name}</span>
                      <span style={{ fontSize:10,color:C.text3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:110 }}>{next.step}</span>
                    </> : ws==="Complete" ? <span style={{ fontSize:11,color:C.success }}>Complete ✓</span>
                ...
                {!panelOpen && (
                  <div style={{ display:"flex",alignItems:"center" }}>
                    {isAdmin&&!hasFilings ? (
                      <Btn size="sm" onClick={async e=>{
                        e.stopPropagation();
                        const types=getFilingTypes(c.jurisdiction);
                        for(const ft of types){
                          const steps=getSteps(ft,c.jurisdiction).map((name,i)=>({stepId:uid(),stepName:name,assignedTo:"",status:"Pending",notes:"",completedAt:"",order:i}));
                          ...
                          await fbSaveFiling(f);
                          updateFiling(c.name,f);
                        }
                        showToast("Filings created for "+c.name);
                      }}>+ Create</Btn>
                    ) : <span style={{ fontSize:11,color:"#818cf8" }}>View →</span>}
                  </div>
                )}
              </div>
            );
        ...
        {/* Side Panel */}
        {selectedCo && (
          <SidePanel company={selectedCo} filings={filings} users={users} isAdmin={isAdmin}
            currentUserEmail={currentUser?.email} yearFilter={yearFilter}
            onClose={()=>setSelectedCo(null)} updateFiling={updateFiling}/>
        )}
      </div>

      {/* Modals */}
      {emailBlast && <EmailBlastModal companies={companies} filings={filings} yearFilter={yearFilter} onClose={()=>setEmailBlast(false)}/>}
      {yearMgr && <YearManager companies={companies} filings={filings} setFilings={setFilings} yearFilter={yearFilter} setYearFilter={setYearFilter} onClose={()=>setYearMgr(false)} showToast={showToast}/>}
